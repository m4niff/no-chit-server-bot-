...
const hostileMobs = [
  'zombie', 'drowned', 'skeleton', 'creeper', 'spider', 'husk', 'witch',
  'zombified_piglin', 'zoglin', 'phantom', 'vex', 'pillager', 'evoker', 'vindication', 'ravager'
];
...
function createBot() {
  ...
  bot.on('kicked', (reason) => {
    ...
    shutdownBotImmediately(); // Always shut down after kick
  });

  bot.on('error', err => {
    console.log("❗ Bot error:", err.message);
    shutdownBotImmediately(); // Shut down on error
  });

  bot.on('end', () => {
    console.log("🔌 Bot disconnected.");
    stopBotActions();
    shutdownBotImmediately(); // Shut down on disconnect
  });
  ...
  const messages = [
    ...
    "if u wana kno spe sofea hmm i dono", "bising do bal", "ko un sme je man", "apa ko bob",
    "okok ma bad ma fault gng🥀", "sat berak sat", "sunyi siak", "MUSTARRRRRRRDDDDDDDD", "ok aq ulang blik dri awal"
  ];
  let index = 0;
